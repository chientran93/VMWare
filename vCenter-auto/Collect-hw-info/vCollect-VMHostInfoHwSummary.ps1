...
#################################  
$vCenterList = Get-Content "D:\Code\vCenter.txt"
...
foreach ($vCenter in $vCenterList){
  ...
  If($? -Eq $True){
    ...
    Write-Progress -Activity "vCollecting vCenter" -Status ("vCenter: {0}" -f $vCenter) -PercentComplete ($countvc/$TotalVcCount*100) -Id 0
      ...
      $Report = @() 
      $vmHosts = get-vmhost | select Name,ConnectionState,PowerState,NumCpu,MemoryUsageGB,Version,Build,MaxEVCMode,@{N="BiosVersion";E={$_.ExtensionData.Hardware.BiosInfo.BiosVersion}}, @{N="BiosReleaseDate";E={$_.ExtensionData.Hardware.BiosInfo.ReleaseDate}},@{N="Cluster";E={ $_.Parent}},@{N="vCenter";E={ ($_.uid).split("@")[1].split(":")[0]}}, @{N="MemoryTotalGB";E={[math]::Round($_.MemoryTotalGB, 2)}} 
      $i=0
